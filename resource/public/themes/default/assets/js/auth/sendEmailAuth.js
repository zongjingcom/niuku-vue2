...
$(function () {
    ...
    var register = $(".emailauth-form").Validform({
        btnSubmit: "#btn_sub",
        tiptype: 3,
        label: ".label",
        showAllError: true,
    ...
    var handler = function (captchaObj) {
        captchaObj.onReady(function () {
            ...
            if (email) {
                //设置button效果，开始计时
                var token = $('#sendEmailCode').attr('token');
                ...
                $.post('/user/emailcode', {
                    ...
                    if (msg.code == 1001) {
                        ...
                        if (msg.code == 1000) {
                            ...
                            $("#sendEmailCode").text("重新获取(" + curCount + ")");
                            InterValObj = window.setInterval(SetRemainTime, 1000); //启动计时器，1秒执行一次
                        } else {
                            thisPos.html('重新发送');
                ...
                if (!$('input[name="email"]').val()) {
                    $('input[name="email"]').siblings('.Validform_checktip').addClass('Validform_wrong').text('请先输入邮箱地址');
                }
            }
...
function SetRemainTime() {
    if (curCount == 0) {
        ...
        $("#sendEmailCode").removeAttr("disabled"); //启用按钮
        $("#sendEmailCode").text("重新获取");
    } else {
        curCount--;
        $("#sendEmailCode").text("重新获取(" + curCount + ")");
    }
